  ...
  return (
    ...
    <Layout title="Restaurants" action={openNewModal} color="bg-orange-700">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {restaurants.map((restaurant: {name:string, address:string, phone:string, id:number}) => (
          ...
          editAction={() => setSearchParams({ edit: restaurant.id.toString() })}
          deleteAction={(e) => {
                       if (!confirm('¿Está seguro de eliminar este restaurante? Todas las órdenes asociadas también se eliminarán.')) {
                      e.preventDefault();
                       }
                     }}
          />
